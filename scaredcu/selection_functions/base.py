...
class SelectionFunction:
    ...
    def _set_words(self, words):
        if words is not None and not isinstance(words, (int, slice, type(...), list, _cu.ndarray)):
            raise TypeError(f'words should be instance of int, slice, Ellipsis, list or ndarray, not {type(words)}.')

        if isinstance(words, list):
            words = _cu.array(words, dtype='uint8')

        if isinstance(words, _cu.ndarray) and words.dtype.kind not in ('u', 'i'):
...
class _AttackSelectionFunction(SelectionFunction):
    ...
    def __init__(self, function, guesses, words, expected_key_function=None, byte_guesses=True):
        super().__init__(function=function, words=words)
        if isinstance(guesses, range):
            guesses = _cu.array(guesses, dtype='uint8')
            byte_guesses = True
        if byte_guesses:
...
class _AttackSelectionFunctionWrapped(_AttackSelectionFunction):
    ...
    def __init__(self, function, guesses, words, expected_key_function=None, target_tag=None, key_tag=None, target_name='data', key_name='key', byte_guesses=True):
        super().__init__(function=function, words=words, guesses=guesses, expected_key_function=expected_key_function, byte_guesses=byte_guesses)
