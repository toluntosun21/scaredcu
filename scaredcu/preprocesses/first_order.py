...
def square(traces):
    ...
    """
    return _cu.square(traces, dtype=max(traces.dtype, 'float32'))
...
def serialize_bit(traces):
    ...
    """
    return _cu.unpackbits(traces.astype('uint8'), axis=1)
...
def fft_modulus(traces):
    ...
    """
    length = _cu.ceil(traces.shape[1] / 2).astype('uint32')
    return _cu.abs(_cu.fft.fft(traces))[:, :length]
...
def center(traces):
    ...
    """
    return _center(traces, _cu.nanmean(traces, axis=0, dtype=max(traces.dtype, 'float32')))
...
def standardize(traces):
    ...
    """
    return center(traces) / _cu.nanstd(traces, axis=0, dtype=max(traces.dtype, 'float32'))
...
class StandardizeOn(Preprocess):
    ...
    def __init__(self, mean=None, std=None, precision='float32'):
        self.mean = mean
        self.std = std
        self.precision = _cu.dtype(precision)

    def __call__(self, traces):
        precision = max(traces.dtype, self.precision)
        _mean = self.mean if self.mean is not None else _cu.nanmean(traces, axis=0, dtype=precision)
        _std = self.std if self.std is not None else _cu.nanstd(traces, axis=0, dtype=precision)
        try:
...
class ToPower(Preprocess):
    ...
    def __call__(self, traces):
        return _cu.power(traces, self.power, dtype=max(traces.dtype, self.precision))
