...
ROUND_KEY_BITS_INDEXES = _cu.array([
    # Round key 0
    [
        [9, 50, 33, 59, 48, 16],
        [32, 56, 1, 8, 18, 41],
...
ROUND_KEY_MISSING_BITS_INDEXES = _cu.array([
...
def key_schedule(key, interrupt_after_round=15):
    ...
    key_bits = _cu.empty((key.shape[0], 64), dtype=_cu.uint8)
    # split the key into bits
    for current_key_byte in _cu.arange(8):
    ...
    output_key = _cu.zeros((key.shape[0], 8 * (interrupt_after_round + 1)), dtype=_cu.uint8)
    for current_round in _cu.arange(16):
        for current_word in _cu.arange(8):
...
def get_master_key(round_key, nb_round, plaintext, expected_ciphertext):
    ...
    for guess in guess_keys:
        ...
        if _cu.array_equal(expected_ciphertext, computed_ciphertext):
            return guess
    return None
...
def _find_possible_keys(round_key, nb_round):
    ...
    ci_di = _cu.roll(ci_di, + nb_shift[nb_round], 1)
    ...
    return _cu.array([[(guess >> (8 * hit)) & 0xFF for hit in range(7, -1, -1)] for guess in guessed_keys], dtype='uint8')
...
def initial_permutation(state):
    ...
    out = _cu.zeros((data.shape[0], 8), dtype=_cu.uint8)
    for current_byte in _cu.arange(8):
        out[:, 0] <<= 1
...
def expansive_permutation(state):
    ...
    for current_2byte_block in _cu.arange(4):
...
def add_round_key(state, keys):
    ...
    return _cu.bitwise_xor(state, keys)
...
def sboxes(state):
    ...
    dimensions = state.shape
    data = state.reshape((-1, 8))
    out = _cu.empty((data.shape[0], 8), dtype=_cu.uint8)
...
def permutation_p(state):
    ...
    dimensions = state.shape
    data = state.reshape((-1, 8))
    out = _cu.empty((data.shape[0], 4), dtype=_cu.uint8)
...
def inv_permutation_p(state):
    ...
    dimensions = state.shape
    data = state.reshape((-1, 4))
    out = _cu.zeros((data.shape[0], 8), dtype=_cu.int64)
    ...
    return out.reshape(dimensions[:-1] + (8,)).astype(_cu.uint8)
...
class _ParametricCipher():
    ...
    def parametric_cipher(self, state, key, after_step, at_round=None, at_des=None, mode='encrypt'):
        ...
        if state.ndim == 1:
            out_state = _cu.array([_cu.copy(state) for i in range(des_keys.shape[1])], dtype='uint8').reshape(-1, 8)
        else:
            out_state = _cu.array(state, dtype='uint8', copy=True)
    ...
    def _parametric_cipher_step(self, out_state, operation, des_keys, des_number, round_number):  # noqa: C901
        if operation is Steps.INITIAL_PERMUTATION:
            ...
            self.saved_left_right = _cu.copy(out_state)
            ...
            out_state[:, 4:8] = _cu.zeros((self.total_words, 4), dtype=_cu.uint8)
        elif operation is Steps.XOR_WITH_SAVED_LEFT_RIGHT:
            out_state = _cu.bitwise_xor(out_state, self.saved_left_right)
        elif operation is Steps.PERMUTE_RIGHT_LEFT:
            out_state = _cu.roll(out_state, shift=4, axis=-1)
            ...
            out_state = inv_permutation_p(_cu.bitwise_xor(out_state[:, 0:4], out_state[:, 4:8]))
    ...
    def _prepare_keys(self, key, state):
        ...
        for current_des in range(self.at_des + 1):
            ...
            if (self.mode == 'encrypt' and current_des == 1) or (self.mode == 'decrypt' and current_des != 1):
                expanded_keys[-1] = _cu.flip(expanded_keys[-1], axis=1)

        # we finally return a 4 dimension numpy array (des number, number of keys, rounds, bytes)
        return _cu.array(expanded_keys, dtype=_cu.uint8)
