...
class NTT:
    ...
    def __init__(self, reduction, n, root):
        ...
        self.ntt_mat = _cu.zeros((self.n,self.n),dtype='uint16')
        for i in range(self.n):
            for j in range(self.n):
                self.ntt_mat[i,j] = pow(self.root, (2*bit_reverse_table[i] + 1)*j, self.q)

        self.ntt_mat_inv = _cu.zeros((self.n,self.n), dtype='uint16')
    ...
    def ntt(self, a):
        t = (_cu.matmul(self.ntt_mat.astype('uint64'), a[::2].astype('int64')) % self.q).astype('uint16')
        return self.reduction.reduce(t)

    def ntt_inv(self, a):
        t = (_cu.matmul(self.ntt_mat_inv.astype('uint64'), a[::2].astype('int64')) % self.q).astype('uint16')
        return self.reduction.reduce(t)
...
class BaseMulIncomplete:
    ...
    def basemul(self, a, b, frame=None, low=True, high=True):
        ...
        if (not low) or (not high):
            ...
            r = _cu.empty(shape=a.shape, dtype=self.dtype) if self.reduce else _cu.empty(shape=a.shape, dtype=self.mult_dtype)
