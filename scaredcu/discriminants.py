...
def discriminant(function):
    ...
    def disc(data, axis=-1):
        ...
        """
        if not isinstance(data, _cu.ndarray) or data.dtype.kind not in ('b', 'i', 'u', 'f', 'c'):
            raise TypeError(f'data must be a numeric array, not {data}.')

        if axis == -1:
            axis = len(data.shape) - 1
        results = function(data, axis=axis)
        if not isinstance(results, _cu.ndarray) or results.dtype.kind not in ('b', 'i', 'u', 'f', 'c'):
...
def nanmax(data, axis):
    ...
    """
    return _cu.nanmax(data, axis=axis)
...
def maxabs(data, axis):
    ...
    """
    return _cu.nanmax(_cu.absolute(data), axis=axis)
...
def nansum(data, axis):
    ...
    """
    return _cu.nansum(data, axis=axis)
...
def abssum(data, axis):
    ...
    """
    return _cu.nansum(_cu.abs(data), axis=axis)
