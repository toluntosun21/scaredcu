...
SHIFT_ROWS = _cu.array([0, 5, 10, 15, 4, 9, 14, 3, 8, 13, 2, 7, 12, 1, 6, 11], dtype=_cu.uint8)
INV_SHIFT_ROWS = _cu.array([0, 13, 10, 7, 4, 1, 14, 11, 8, 5, 2, 15, 12, 9, 6, 3], dtype=_cu.uint8)
...
def _expand_forward(key_cols, bytes_key_length, cols_in, number_of_keys, col_in, col_out):
    ...
    expanded_key = _cu.empty((number_of_keys, col_out, 4), dtype='uint8')
    final_shape = (number_of_keys, (n_cols * 4))
    for index, col in enumerate(cols_range):
        if index < cols_in:
            expanded_key[:, col] = key[:, index, :]
        elif col % cols_in == 0:
            expanded_key[:, col] = SBOX[_cu.roll(expanded_key[:, col - 1], shift=-1, axis=-1)]
            expanded_key[:, col] = _cu.bitwise_xor(expanded_key[:, col], RCON[int(col / cols_in) - 1])
            expanded_key[:, col] = _cu.bitwise_xor(expanded_key[:, col], expanded_key[:, col - cols_in])
        elif bytes_key_length == 32 and col % 4 == 0:
            expanded_key[:, col] = _cu.bitwise_xor(
                SBOX[expanded_key[:, col - 1]],
                expanded_key[:, col - cols_in]
            )
        else:
            expanded_key[:, col] = _cu.bitwise_xor(expanded_key[:, col - 1], expanded_key[:, col - cols_in])
...
def _expand_backward(key_cols, bytes_key_length, cols_in, number_of_keys, col_in, col_out):
    ...
    key = key_cols.reshape((-1, cols_in, 4))
    expanded_key = _cu.empty((number_of_keys, col_in + cols_in, 4), dtype='uint8')
    final_shape = (number_of_keys, (n_cols * 4))
    for index, col in enumerate(cols_range):
        if index < cols_in:
            expanded_key[:, col] = key[:, cols_in - index - 1, :]
        elif col % cols_in == 0:
            expanded_key[:, col] = _cu.bitwise_xor(expanded_key[:, col + cols_in], SBOX[_cu.roll(expanded_key[:, col + cols_in - 1], shift=-1, axis=-1)])
            expanded_key[:, col] = _cu.bitwise_xor(expanded_key[:, col], RCON[int(col / cols_in)])
        elif bytes_key_length == 32 and col % 4 == 0:
            expanded_key[:, col] = _cu.bitwise_xor(
                SBOX[expanded_key[:, col + cols_in - 1]],
                expanded_key[:, col + cols_in])
        else:
            expanded_key[:, col] = _cu.bitwise_xor(expanded_key[:, col + cols_in], expanded_key[:, col + cols_in - 1])
...
def mix_column(vectors):
    ...
    out = _cu.zeros(data.shape, dtype='uint8')
    for row in range(4):
        tmp = _cu.array([
            XTIME_2[data[:, row]],
            data[:, row],
        ...
        tmp = _cu.roll(tmp, shift=row, axis=-1)
        out = _cu.bitwise_xor(out, tmp)
...
def mix_columns(state):
    ...
    out = _cu.empty(data.shape, dtype='uint8')
    for col in range(4):
...
def inv_mix_column(vectors):
    ...
    dims = vectors.shape
    data = vectors.reshape((-1, 4))
    out = _cu.zeros(data.shape, dtype=_cu.uint8)
    for row in range(4):
        tmp = _cu.array([
            XTIME_14[data[:, row]],
            XTIME_9[data[:, row]],
...
def _parametric_cipher(state, key, operations, after_step, at_round=None, mode='encrypt'):
    ...
    if state.ndim == 1:
        out_state = _cu.array([_cu.copy(state) for i in range(round_keys.shape[0])], dtype='uint8')
    else:
...
def _prepare_keys(key, state, mode):
    ...
    if mode == 'decrypt':
        round_keys = _cu.flip(round_keys, axis=1)
    return round_keys
