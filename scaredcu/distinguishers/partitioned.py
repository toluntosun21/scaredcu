...
class _PartitionnedDistinguisherBaseMixin(DistinguisherMixin):
    ...
    def _init_partitions(self, data):
        maxdata = _cu.nanmax(data)
        mindata = _cu.nanmin(data)
        if self.partitions is None:
            ...
            for r in ls:
                if maxdata <= r:
                    break
            self.partitions = _cu.arange(r, dtype='int32')
    ...
    def _update(self, traces, data):
        ...
        if not _cu.issubdtype(data.dtype, _cu.integer):
...
def _define_lut_func(partitions):
    lut = _cu.zeros(2**17, dtype='int32') - 1
    _build_lut[32,32](partitions, lut)
    #@_cu.vectorize([_cu.int32(_cu.uint8), _cu.int32(_cu.uint16), _cu.int32(_cu.uint32), _cu.int32(_cu.uint64),
    #                _cu.int32(_cu.int8), _cu.int32(_cu.int16), _cu.int32(_cu.int32), _cu.int32(_cu.int64)])
    # @_cu.fuse
    def _lut_function(x):
        return lut[x]
...
class PartitionedDistinguisherMixin(_PartitionnedDistinguisherBaseMixin):
    ...
    def _initialize_accumulators(self):
        ...
        self.sum_square = _cu.zeros((self._trace_length, self._data_words, len(self.partitions)), dtype=self.precision)
    ...
    def _accumulate_core_1(traces, data, self_sum, self_sum_square, self_counters, self_precision):
        for sample_idx in _nb.prange(traces.shape[1]):
            tmp_sum = _cu.zeros((self_counters.shape[0], self_counters.shape[1]), dtype='float64')
            tmp_sum_square = _cu.zeros((self_counters.shape[0], self_counters.shape[1]), dtype='float64')
    ...
    def _accumulate_core_2(traces, data, self_sum, self_sum_square, self_counters, self_precision):
        ...
        bool_mask = _cu.empty((traces.shape[0], data.shape[1] * self_counters.shape[1]), dtype=self_precision)
    ...
    def _accumulate(self, traces, data):
        ...
        if len(self.partitions) > 9:
            ...
            if not hasattr(self, '_timings'):
                self._timings = [-2, -1]
            function_idx = _cu.argmin(self._timings)
    ...
    def _compute(self):
        ...
        result = _cu.empty((self._data_words, self._trace_length), dtype=self.precision)
...
def _set_partitions(obj, partitions):
    if partitions is not None:
        if not isinstance(partitions, (_cu.ndarray, list, range)):
            raise TypeError(f'partitions should be a ndarray, list or range instance, not {type(partitions)}.')
        if not isinstance(partitions, _cu.ndarray):
            partitions = _cu.array(partitions, dtype='int32')
        elif partitions.dtype.kind not in 'iu':
            raise ValueError(f'partitions should be an integer array, not {partitions.dtype}.')
        if _cu.max(partitions) >= 2**16:
            raise ValueError(f'partition values must be in ]-2^16, 2^16[, but {_cu.max(partitions)} found.')
        if _cu.min(partitions) <= -2**16:
            raise ValueError(f'partition values must be in ]-2^16, 2^16[, but {_cu.min(partitions)} found.')
...
class ANOVADistinguisherMixin(PartitionedDistinguisherMixin):
    ...
    def _compute_metric(self, non_zero_indices, non_zero_counters, sums, sums_squared, number_non_zero):
        total_non_empty_partitions = _cu.count_nonzero(non_zero_indices)

        partitions_means = (sums / non_zero_counters)
        mean = _cu.sum(sums, axis=-1, keepdims=True) / number_non_zero
...
class NICVDistinguisherMixin(PartitionedDistinguisherMixin):
    ...
    def _compute_metric(self, non_zero_indices, non_zero_counters, sums, sums_squared, number_non_zero):
        mean = _cu.sum(sums, axis=1) / number_non_zero

        numerator = (((sums / non_zero_counters).T - mean).T)**2
        numerator *= non_zero_counters / number_non_zero
        numerator = _cu.sum(numerator, axis=1)

        denominator = _cu.sum(sums_squared, axis=1) / number_non_zero - (mean)**2
...
class SNRDistinguisherMixin(PartitionedDistinguisherMixin):
    ...
    def _compute_metric(self, non_zero_indices, non_zero_counters, sums, sums_squared, number_non_zero):
        mean = _cu.sum(sums, axis=1) / number_non_zero
        numerator = (((sums / non_zero_counters).T - mean).T)**2
        numerator = _cu.sum(numerator, axis=1) / non_zero_indices.shape[0]

        denominator = (sums_squared / non_zero_counters) - (sums / non_zero_counters)**2
        denominator = _cu.sum(denominator, axis=1) / non_zero_indices.shape[0]
