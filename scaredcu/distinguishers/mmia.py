...
class MMIADistinguisherMixin(MIADistinguisherMixin):
    ...
    def _init_bin_edges(self, traces):
        if self.same_bins:
            if self.bin_edges is None and self.bin_edgesx2 is None:
                self.y_window = (min(_cu.min(traces[:, :self.offset]), _cu.min(traces[:, -self.offset:])),
                                 max(_cu.max(traces[:, :self.offset]), _cu.max(traces[:, -self.offset:])))
            ...
            if self.bin_edgesx2 is None:
                self.y_windowx2 = (_cu.min(traces[:, -self.offset:]), _cu.max(traces[:, -self.offset:]))
                self.bin_edgesx2 = _cu.linspace(*self.y_windowx2, self.bins_numberx2 + 1)

    def _initialize_accumulators(self):
        self.accumulators = _cu.zeros((self._trace_length - self.offset, self.bins_number, self.bins_numberx2,
                                       len(self.partitions), self._data_words),
                                      dtype=self.precision)

    @staticmethod
    @_cuda.jit(cache=True)
    def _accumulate_core(traces, data, self_bin_edges, self_accumulators, offset, self_bin_edgesx2,
                         self_place_outliers):
        start = _cuda.grid(1)
    ...
    def _accumulate(self, traces, data):
        self._accumulate_core[64,8](traces, data, self.bin_edges, self.accumulators, self.offset, self.bin_edgesx2,
                              self.place_outliers)
    ...
    def compute(self):
        try:
            if self.last_trace_num == self.processed_traces:
                return self.results
            else:
                self.last_trace_num = self.processed_traces
...
def _set_histogram_parameters(obj, bins_number, bin_edges, offset, same_bins, bin_edgesx2, bins_numberx2, place_outliers):
    ...
    _set_histogram_parameters_mia(obj, bins_number=bins_number, bin_edges=bin_edges)
    obj.offset = offset
