...
class _TemplateBuildDistinguisherMixin(partitioned._PartitionnedDistinguisherBaseMixin):
    ...
    def _initialize_accumulators(self):
        self._exi = _cu.zeros(shape=(len(self.partitions), self._trace_length), dtype=self.precision, order='C')
        self._exxi = _cu.zeros(shape=(len(self.partitions), self._trace_length, self._trace_length), dtype=self.precision, order='C')
        self._counters = _cu.zeros(shape=(len(self.partitions)), dtype=self.precision)
    ...
    def _accumulate(self, traces, data):
        ...
        bool_mask = _cu.empty((len(self.partitions), len(data)), dtype='bool')
        for p in range(len(self.partitions)):
            bool_mask[p] = data[:, 0] == p  # Data are already transformed to correspond to partition indexes
        self._counters += _cu.sum(bool_mask, axis=1)
        traces = traces.astype(self.precision)
        self._exi += _cu.dot(bool_mask, traces)
        for p in range(len(self.partitions)):
            tmp = traces[bool_mask[p]]
            tmp = _cu.dot(tmp.T, tmp)
            self._exxi[p] += tmp

    def _compute(self):
        self.pooled_covariance = _cu.zeros((self._trace_length, self._trace_length))
        self.pooled_covariance_inv = _cu.empty((self._trace_length, self._trace_length))

        tmp_counters = _cu.copy(self._counters).astype(self.precision)
        if _cu.any(tmp_counters <= 1):
        ...
        for i, p in enumerate(self.partitions):
            tmp_matrix = (_cu.outer(templates[i], templates[i]) * tmp_counters[i])
            self.pooled_covariance += (self._exxi[i] - tmp_matrix) / (tmp_counters[i] - 1)

        self.pooled_covariance /= len(self.partitions)
        self.pooled_covariance_inv = _cu.linalg.pinv(self.pooled_covariance)
        return templates
...
class _BaseTemplateAttackDistinguisherMixin(base.DistinguisherMixin, partitioned.PartitionedDistinguisherBase):
    ...
    def _initialize(self, traces, data):
        ...
        self._scores = _cu.zeros(shape=(self._get_dimension(traces, data), ), dtype=self.precision)
    ...
    def _update(self, traces, data):
        ...
        for i in range(self._get_dimension(traces, data)):
            ...
            tmp = _cu.dot(tmp_traces, self.pooled_covariance_inv)
            tmp = tmp * tmp_traces
            scores.append(tmp.sum())
        self._scores += _cu.array(scores) / traces.shape[1]
