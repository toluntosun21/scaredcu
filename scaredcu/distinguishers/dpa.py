...
class DPADistinguisherMixin(DistinguisherMixin):
    ...
    def _initialize(self, traces, data):
        max_data = _cu.nanmax(data)
        trace_size = traces.shape[1]
        data_words = data.shape[1]
        if not isinstance(max_data, (int, _cu.uint8)):
        ...
        try:
            self.accumulator_traces = _cu.zeros((trace_size), dtype=self.precision)
            self.accumulator_ones = _cu.zeros((data_words, trace_size), dtype=self.precision)
            self.processed_ones = _cu.zeros((data_words), dtype='uint32')
    ...
    def _update(self, traces, data):
        ...
        self.processed_ones += _cu.sum(data, axis=0)
        traces = traces.astype(self.precision, order='F')
        data = data.astype(self.precision, order='F')
        self.accumulator_traces += _cu.sum(traces, axis=0)
        self.accumulator_ones += _cu.dot(data.T, traces)
