...
class CPADistinguisherMixin(DistinguisherMixin):
    ...
    def _initialize(self, traces, data):
        ...
        try:
            self.ex = _cu.zeros((trace_size), dtype=self.precision)
            self.ex2 = _cu.zeros((trace_size), dtype=self.precision)
            self.ey = _cu.zeros((data_words), dtype=self.precision)
            self.ey2 = _cu.zeros((data_words), dtype=self.precision)
            self.exy = _cu.zeros((data_words, trace_size), dtype=self.precision)
    ...
    def _update(self, traces, data):
        ...
        self.ey += _cu.sum(_data, axis=0)
        self.ey2 += _cu.sum(_data ** 2, axis=0)
        self.ex += _cu.sum(_traces, axis=0)
        self.ex2 += _cu.sum(_traces ** 2, axis=0)
        self.exy += _cu.dot(_data.T, _traces)
        logger.info(f'End updating accumulators for {self.__class__.__name__}.')

    def _compute(self):
        trace_size = self.ex.shape[0]
        data_words = self.ey.shape[0]
        common_1 = _cu.sqrt(self.ex2 - self.processed_traces * ((self.ex / self.processed_traces)**2))
        common_2 = _cu.sqrt(self.ey2 - self.processed_traces * ((self.ey / self.processed_traces)**2))
        result = _cu.empty((data_words, trace_size), dtype=self.precision)
...
class CPAAlternativeDistinguisherMixin(CPADistinguisherMixin):
    ...
    def _compute(self):
        sigma_traces = _cu.sqrt(self.processed_traces * self.ex2 - (self.ex) ** 2)
        sigma_data = _cu.sqrt(self.processed_traces * self.ey2 - (self.ey) ** 2)
        enum = self.processed_traces * self.exy - _cu.matmul(self.ey[:, None], self.ex[None, :])
        denom = _cu.matmul(sigma_data[:, None], sigma_traces[None, :])
        return (enum / denom)
